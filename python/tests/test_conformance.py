"""The load-bearing test: each eDSL module must produce IR proto-equal
to its committed gallery `ir.json` — the canonicalized artifact
(`scripts/gen-examples.sh`) must never drift from the eDSL source that
generates it."""
...
def test_module_main_emits_parseable_json() -> None:
    ...
    assert json_format.Parse(out, ir_pb2.Ir()) == EthIpvxL4.to_pb()
