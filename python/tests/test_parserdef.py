"""ParserDef machinery: the class-ness must pay rent.
...
from pakeles import ParserDef, StateChain, extract, reject
...
class Rung1(Rung0):
    ...
    def parse_ethernet(self) -> StateChain:  # override: add the VLAN arm
        return extract(Ethernet).select(
            Ethernet.ethertype,
...
def test_rung_inheritance() -> None:
    r0 = json.loads(Rung0.build().to_json())["parser"]
    ...
    r1 = json.loads(Rung1.build().to_json())["parser"]
...
class NoEncap(LinuxFlowDissector):
    ...
    def parse_ipip(self) -> StateChain:
        return StateChain().then(reject("encap disabled"))
...
def test_variant_override() -> None:
    base = json.loads(LinuxFlowDissector.build().to_json())["parser"]
    var = json.loads(NoEncap.build().to_json())["parser"]
