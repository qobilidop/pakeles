...
static int adapt(const pk_dpdk_ptype_result_t *r, proj_t *o) {
    if (r->outcome != PK_DPDK_PTYPE_ACCEPT) return -1;
    memset(o, 0, sizeof *o);
    uint64_t bits = 0;
    int used_ip6_opt = 0, used_frag = 0;

    if (!r->ethernet_present) return -1;
    ...
    for (;;) {
        ...
        if (proto == 0x0800 && r->ipv4_present && !inner) {
            ...
            if (vihl == 0x45) o->ptype |= RTE_PTYPE_L3_IPV4;
            else if (vihl >= 0x46 && vihl <= 0x4F) o->ptype |= RTE_PTYPE_L3_IPV4_EXT;
            ...
            const pk_dpdk_ptype_ipv4_t *ip = &r->ipv4;
            uint8_t vihl = (uint8_t)(ip->version << 4 | ip->ihl);
            if (vihl == 0x45) o->ptype |= RTE_PTYPE_INNER_L3_IPV4;
            else if (vihl >= 0x46 && vihl <= 0x4F) o->ptype |= RTE_PTYPE_INNER_L3_IPV4_EXT;
        ...
        if (!inner) {
            ...
            if (proto == 17) { o->ptype |= RTE_PTYPE_L4_UDP; o->hl.l4_len = 8; goto audit; }
            if (proto == 132) { o->ptype |= RTE_PTYPE_L4_SCTP; o->hl.l4_len = 12; goto audit; }
            // Tunnel arms (incl. the LE byte-swap EtherTypes).
            if (proto == 47 || proto == 0x2F00) {
                if (!r->gre_present) return -1;
                bits += 32;
                if (r->gre.r) goto audit; // R=1: not a tunnel
                if (!r->gre_opt_present) return -1;
                gre_seen = 1;
                o->hl.tunnel_len =
                    4 + 4u * (r->gre.c + r->gre.k + r->gre.s);
            ...
            // Inner L2.
            if (proto == 0x6558) {
                if (!r->ethernet_present) return -1; // shared slot: repeat
                return -1; // inner Ethernet reuses the ethernet slot — lost
            }
            if (proto == 0x8100) {
                if (!r->vlan_present || o->hl.l2_len > 14) return -1; // slot reuse check
                o->ptype = (o->ptype & ~RTE_PTYPE_INNER_L2_MASK) | RTE_PTYPE_INNER_L2_ETHER_VLAN;
                o->hl.inner_l2_len += 4;
                bits += 32;
                proto = r->vlan.proto;
            } else if (proto == 0x88A8) {
                if (!r->qinq_present || o->hl.l2_len > 14) return -1;
                o->ptype = (o->ptype & ~RTE_PTYPE_INNER_L2_MASK) | RTE_PTYPE_INNER_L2_ETHER_QINQ;
                o->hl.inner_l2_len += 8;
                bits += 64;
                proto = r->qinq.proto;
            }
...
audit:
    ...
    if (bits != r->consumed_bits) return -1;
    if (gre_seen && o->hl.tunnel_len != 4 + r->gre_opt.body_bit_len / 8) return -1;
    return 0;
}
...
int main(int argc, char **argv) {
    if (argc < 2) { fprintf(stderr, "usage: %s <corpus.txt> [bench_iters]\n", argv[0]); return 2; }
    ...
    while (fgets(line, sizeof line, cf) && n < MAX_PKTS) {
        ...
        if (plen <= 0) continue; // oversized witnesses: bench corpus only
        lens[n++] = plen;
    }
    fclose(cf);
    ...
    for (int i = 0; i < n; i++) {
        ...
        proj_t got;
        if (adapt(&res, &got) != 0) { skipped++; continue; }
        projected++;
        if (got.ptype != want || memcmp(&got.hl, &hl, sizeof hl) != 0) {
            mismatches++;
            printf("  MISMATCH pkt %d: ours=%#x dpdk=%#x (lens %u/%u/%u/%u/%u/%u/%u vs %u/%u/%u/%u/%u/%u/%u)\n",
                   i, got.ptype, want,
                   got.hl.l2_len, got.hl.l3_len, got.hl.l4_len, got.hl.tunnel_len,
                   got.hl.inner_l2_len, got.hl.inner_l3_len, got.hl.inner_l4_len,
                   hl.l2_len, hl.l3_len, hl.l4_len, hl.tunnel_len,
                   hl.inner_l2_len, hl.inner_l3_len, hl.inner_l4_len);
        }
    }
    printf("adapter: %d projected (all must match), %d skipped (reject paths + repeated-instance shapes), %d mismatches\n",
           projected, skipped, mismatches);

    // 3. Benchmark: whole-corpus round-robin.
    if (iters <= 0) return mismatches == 0 ? 0 : 1;
