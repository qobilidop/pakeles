...
Everything in this directory comes from **one description** — authored
in the Python eDSL ([`eth_ipv4_tcp.py`](eth_ipv4_tcp.py)) or the Rust
builder ([`src/examples.rs`](../../src/examples.rs)), which are
proto-equality-tested to agree — and is equality-guarded by tests: if
any file here drifts from what the toolchain generates, CI fails.
Regenerate with `./dev.sh cargo run --bin gen_examples`.

| File | What it is | Generated by |
|---|---|---|
| [`eth_ipv4_tcp.py`](eth_ipv4_tcp.py) | **The input**: the description as authored in the Python eDSL | (authored; mirrored from `py/`) |
