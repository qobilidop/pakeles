...
- **`parse_first`** — extracts first_byte; selects on `first_byte.form`:
  - first_byte.form == 0x0 → `short_accept`
...
- **`short_accept`**; sets meta.kind = 7; then **accept**
...
- **`parse_scid`** — extracts scid; selects on `first_byte.ty`:
  - first_byte.ty == 0x0 → `initial_mark`
  - first_byte.ty == 0x1 → `zero_rtt_mark`
  - first_byte.ty == 0x2 → `handshake_mark`
  - first_byte.ty == 0x3 → `retry_accept`
  - otherwise → **reject** (*unreachable: 2-bit key*)
  > CIDs done; the long-type bits pick the tail grammar.
- **`initial_mark`**; sets meta.kind = 1; then `parse_tok_lead`
- **`zero_rtt_mark`**; sets meta.kind = 2; then `parse_len_lead`
  > No token by grammar; straight to the length varint (quinn's
  > parse extent - quiche stops after the SCID here).
- **`handshake_mark`**; sets meta.kind = 3; then `parse_len_lead`
- **`retry_accept`**; sets meta.kind = 4; then **accept**
