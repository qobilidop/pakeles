/* Generated by pakeles from `quic_initial` (eBPF variant). Do not edit:
 * regenerate with `pakeles gen bpf`.
 * Compile: clang -O2 -target bpf -c this_file.c
 * Entry contract (rbpf raw VM): r1 = mem = 8-byte LE bit_len ++ packet;
 * the length prefix is the harness framing (rbpf passes no length).
 * Returns outcome(8b) << 56 | reason(8b) << 48 | consumed_bits(48b).
 * Note: the result struct lives on the (512-byte) BPF stack;
 * large parsers will need a redesign. */

/* Freestanding: no libc for the bpf target. */
typedef __UINT8_TYPE__ uint8_t;
typedef __UINT16_TYPE__ uint16_t;
typedef __UINT32_TYPE__ uint32_t;
typedef __UINT64_TYPE__ uint64_t;

typedef struct {
  uint8_t form;
  uint8_t fixed;
  uint8_t ty;
  uint8_t low;
} pk_quic_initial_first_byte_t;

typedef struct {
  uint32_t v;
} pk_quic_initial_version_t;

typedef struct {
  uint8_t dcid_len;
  uint64_t dcid_bit_off;
  uint64_t dcid_bit_len;
  uint8_t scid_len;
  uint64_t scid_bit_off;
  uint64_t scid_bit_len;
} pk_quic_initial_other_cids_t;

typedef struct {
  uint8_t ln;
} pk_quic_initial_dcid_len_t;

typedef struct {
  uint64_t cid_bit_off;
  uint64_t cid_bit_len;
} pk_quic_initial_dcid_t;

typedef struct {
  uint8_t ln;
} pk_quic_initial_scid_len_t;

typedef struct {
  uint64_t cid_bit_off;
  uint64_t cid_bit_len;
} pk_quic_initial_scid_t;

typedef struct {
  uint8_t prefix;
  uint8_t v6;
} pk_quic_initial_tok_lead_t;

typedef struct {
  uint64_t body_bit_off;
  uint64_t body_bit_len;
} pk_quic_initial_tok0_t;

typedef struct {
  uint8_t t;
  uint64_t body_bit_off;
  uint64_t body_bit_len;
} pk_quic_initial_tok1_t;

typedef struct {
  uint32_t t;
  uint64_t body_bit_off;
  uint64_t body_bit_len;
} pk_quic_initial_tok2_t;

typedef struct {
  uint64_t t;
  uint64_t body_bit_off;
  uint64_t body_bit_len;
} pk_quic_initial_tok3_t;

typedef struct {
  uint8_t prefix;
  uint8_t v6;
} pk_quic_initial_len_lead_t;

typedef struct {
  uint8_t t;
} pk_quic_initial_len1_t;

typedef struct {
  uint32_t t;
} pk_quic_initial_len2_t;

typedef struct {
  uint64_t t;
} pk_quic_initial_len3_t;

typedef struct {
  uint8_t outcome; /* 0 = accept, 1 = reject */
  uint16_t reason; /* pk_quic_initial_reason */
  uint64_t consumed_bits;
  uint8_t first_byte_present;
  pk_quic_initial_first_byte_t first_byte;
  uint8_t version_present;
  pk_quic_initial_version_t version;
  uint8_t other_cids_present;
  pk_quic_initial_other_cids_t other_cids;
  uint8_t dcid_len_present;
  pk_quic_initial_dcid_len_t dcid_len;
  uint8_t dcid_present;
  pk_quic_initial_dcid_t dcid;
  uint8_t scid_len_present;
  pk_quic_initial_scid_len_t scid_len;
  uint8_t scid_present;
  pk_quic_initial_scid_t scid;
  uint8_t tok_lead_present;
  pk_quic_initial_tok_lead_t tok_lead;
  uint8_t tok0_present;
  pk_quic_initial_tok0_t tok0;
  uint8_t tok1_present;
  pk_quic_initial_tok1_t tok1;
  uint8_t tok2_present;
  pk_quic_initial_tok2_t tok2;
  uint8_t tok3_present;
  pk_quic_initial_tok3_t tok3;
  uint8_t len_lead_present;
  pk_quic_initial_len_lead_t len_lead;
  uint8_t len1_present;
  pk_quic_initial_len1_t len1;
  uint8_t len2_present;
  pk_quic_initial_len2_t len2;
  uint8_t len3_present;
  pk_quic_initial_len3_t len3;
  uint64_t m_kind;
  uint64_t m_token_len;
  uint64_t m_length;
} pk_quic_initial_result_t;

typedef enum {
  PK_R_NONE = 0,
  PK_R_OUT_OF_BOUNDS = 1,
  PK_R_MAX_DEPTH_EXCEEDED = 2,
  PK_R_NO_MATCHING_SELECT_ARM = 3,
  PK_R_OUT_OF_REGION_BOUNDS = 4,
  PK_R_REGION_OUT_OF_BOUNDS = 5,
  PK_R_REGION_NOT_EXHAUSTED = 6,
  PK_R_DCID_TOO_LONG_FOR_V1 = 16,
  PK_R_SCID_TOO_LONG_FOR_V1 = 17,
  PK_R_UNREACHABLE__1_BIT_KEY = 18,
  PK_R_UNREACHABLE__2_BIT_KEY = 19,
} pk_quic_initial_reason_t;

/* Buffer contract: `buf` must hold at least PK_BUF_MASK + 1 bytes.
 * Every packet index is masked with it — dead at runtime (the
 * length guards already bound each access), but it bounds the index
 * register directly, which is what the kernel verifier tracks. */
#ifndef PK_BUF_MASK
#define PK_BUF_MASK 4095u
#endif

static __attribute__((always_inline)) uint64_t pk_read_bits(const uint8_t *buf, uint64_t avail, uint64_t off, uint32_t n) {
  (void)avail;
  uint64_t v = 0;
  uint32_t i;
  for (i = 0; i < n; i++) {
    uint64_t pos = off + i;
    v = (v << 1) | (uint64_t)((buf[(pos >> 3) & PK_BUF_MASK] >> (7 - (pos & 7))) & 1);
  }
  return v;
}

#define PK_S_PARSE_FIRST 0
#define PK_S_SHORT_ACCEPT 1
#define PK_S_PARSE_VERSION 2
#define PK_S_VN_MARK 3
#define PK_S_UNKNOWN_MARK 4
#define PK_S_PARSE_OTHER_CIDS 5
#define PK_S_PARSE_DCID_LEN 6
#define PK_S_PARSE_DCID 7
#define PK_S_PARSE_SCID_LEN 8
#define PK_S_PARSE_SCID 9
#define PK_S_INITIAL_MARK 10
#define PK_S_ZERO_RTT_MARK 11
#define PK_S_HANDSHAKE_MARK 12
#define PK_S_RETRY_ACCEPT 13
#define PK_S_PARSE_TOK_LEAD 14
#define PK_S_TOK_W1 15
#define PK_S_TOK_W2 16
#define PK_S_TOK_W4 17
#define PK_S_TOK_W8 18
#define PK_S_PARSE_LEN_LEAD 19
#define PK_S_LEN_W1 20
#define PK_S_LEN_W2 21
#define PK_S_LEN_W4 22
#define PK_S_LEN_W8 23

static __attribute__((always_inline)) int pk_quic_initial_parse_core(const uint8_t *buf, uint64_t bit_len, pk_quic_initial_result_t *out) {
  uint64_t off = 0;
  uint32_t state = PK_S_PARSE_FIRST;
  uint32_t depth;
  out->m_kind = 0ULL;
  out->m_token_len = 0ULL;
  out->m_length = 0ULL;
  for (depth = 0; depth < 12u; depth++) {
    switch (state) {
    case PK_S_PARSE_FIRST: {
      out->first_byte_present = 1;
      if (off + 1 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->first_byte.form = (uint8_t)pk_read_bits(buf, bit_len, off, 1);
      off += 1;
      if (off + 1 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->first_byte.fixed = (uint8_t)pk_read_bits(buf, bit_len, off, 1);
      off += 1;
      if (off + 2 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->first_byte.ty = (uint8_t)pk_read_bits(buf, bit_len, off, 2);
      off += 2;
      if (off + 4 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->first_byte.low = (uint8_t)pk_read_bits(buf, bit_len, off, 4);
      off += 4;
      uint64_t key0 = (uint64_t)out->first_byte.form;
      if (key0 == 0ULL) {
        state = PK_S_SHORT_ACCEPT;
        continue;
      } else if (key0 == 1ULL) {
        state = PK_S_PARSE_VERSION;
        continue;
      } else {
        out->outcome = 1;
        out->reason = PK_R_UNREACHABLE__1_BIT_KEY;
        out->consumed_bits = off;
        return 1;
      }
    }
    case PK_S_SHORT_ACCEPT: {
      out->m_kind = (7ULL) & 0x7ULL;
      out->outcome = 0;
      out->reason = PK_R_NONE;
      out->consumed_bits = off;
      return 0;
    }
    case PK_S_PARSE_VERSION: {
      out->version_present = 1;
      if (off + 32 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->version.v = (uint32_t)(((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK] << 24) | ((uint64_t)buf[((off >> 3) + 1) & PK_BUF_MASK] << 16) | ((uint64_t)buf[((off >> 3) + 2) & PK_BUF_MASK] << 8) | (uint64_t)buf[((off >> 3) + 3) & PK_BUF_MASK]);
      off += 32;
      uint64_t key0 = (uint64_t)out->version.v;
      if (key0 == 1ULL) {
        state = PK_S_PARSE_DCID_LEN;
        continue;
      } else if (key0 == 0ULL) {
        state = PK_S_VN_MARK;
        continue;
      } else {
        state = PK_S_UNKNOWN_MARK;
        continue;
      }
    }
    case PK_S_VN_MARK: {
      out->m_kind = (5ULL) & 0x7ULL;
      state = PK_S_PARSE_OTHER_CIDS;
      continue;
    }
    case PK_S_UNKNOWN_MARK: {
      out->m_kind = (6ULL) & 0x7ULL;
      state = PK_S_PARSE_OTHER_CIDS;
      continue;
    }
    case PK_S_PARSE_OTHER_CIDS: {
      out->other_cids_present = 1;
      if (off + 8 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->other_cids.dcid_len = (uint8_t)((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK]);
      off += 8;
      {
        uint64_t vlen = (uint64_t)out->other_cids.dcid_len;
        if (vlen > (bit_len - off) / 8) {
          out->outcome = 1;
          out->reason = PK_R_OUT_OF_BOUNDS;
          out->consumed_bits = off;
          return 1;
        }
        out->other_cids.dcid_bit_off = off;
        out->other_cids.dcid_bit_len = vlen * 8;
        off += vlen * 8;
      }
      if (off + 8 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->other_cids.scid_len = (uint8_t)((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK]);
      off += 8;
      {
        uint64_t vlen = (uint64_t)out->other_cids.scid_len;
        if (vlen > (bit_len - off) / 8) {
          out->outcome = 1;
          out->reason = PK_R_OUT_OF_BOUNDS;
          out->consumed_bits = off;
          return 1;
        }
        out->other_cids.scid_bit_off = off;
        out->other_cids.scid_bit_len = vlen * 8;
        off += vlen * 8;
      }
      out->outcome = 0;
      out->reason = PK_R_NONE;
      out->consumed_bits = off;
      return 0;
    }
    case PK_S_PARSE_DCID_LEN: {
      out->dcid_len_present = 1;
      if (off + 8 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->dcid_len.ln = (uint8_t)((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK]);
      off += 8;
      uint64_t key0 = (uint64_t)out->dcid_len.ln;
      if (key0 == 0ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 1ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 2ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 3ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 4ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 5ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 6ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 7ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 8ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 9ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 10ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 11ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 12ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 13ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 14ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 15ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 16ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 17ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 18ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 19ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else if (key0 == 20ULL) {
        state = PK_S_PARSE_DCID;
        continue;
      } else {
        out->outcome = 1;
        out->reason = PK_R_DCID_TOO_LONG_FOR_V1;
        out->consumed_bits = off;
        return 1;
      }
    }
    case PK_S_PARSE_DCID: {
      out->dcid_present = 1;
      {
        uint64_t vlen = (uint64_t)out->dcid_len.ln;
        if (vlen > (bit_len - off) / 8) {
          out->outcome = 1;
          out->reason = PK_R_OUT_OF_BOUNDS;
          out->consumed_bits = off;
          return 1;
        }
        out->dcid.cid_bit_off = off;
        out->dcid.cid_bit_len = vlen * 8;
        off += vlen * 8;
      }
      state = PK_S_PARSE_SCID_LEN;
      continue;
    }
    case PK_S_PARSE_SCID_LEN: {
      out->scid_len_present = 1;
      if (off + 8 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->scid_len.ln = (uint8_t)((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK]);
      off += 8;
      uint64_t key0 = (uint64_t)out->scid_len.ln;
      if (key0 == 0ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 1ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 2ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 3ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 4ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 5ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 6ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 7ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 8ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 9ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 10ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 11ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 12ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 13ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 14ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 15ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 16ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 17ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 18ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 19ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else if (key0 == 20ULL) {
        state = PK_S_PARSE_SCID;
        continue;
      } else {
        out->outcome = 1;
        out->reason = PK_R_SCID_TOO_LONG_FOR_V1;
        out->consumed_bits = off;
        return 1;
      }
    }
    case PK_S_PARSE_SCID: {
      out->scid_present = 1;
      {
        uint64_t vlen = (uint64_t)out->scid_len.ln;
        if (vlen > (bit_len - off) / 8) {
          out->outcome = 1;
          out->reason = PK_R_OUT_OF_BOUNDS;
          out->consumed_bits = off;
          return 1;
        }
        out->scid.cid_bit_off = off;
        out->scid.cid_bit_len = vlen * 8;
        off += vlen * 8;
      }
      uint64_t key0 = (uint64_t)out->first_byte.ty;
      if (key0 == 0ULL) {
        state = PK_S_INITIAL_MARK;
        continue;
      } else if (key0 == 1ULL) {
        state = PK_S_ZERO_RTT_MARK;
        continue;
      } else if (key0 == 2ULL) {
        state = PK_S_HANDSHAKE_MARK;
        continue;
      } else if (key0 == 3ULL) {
        state = PK_S_RETRY_ACCEPT;
        continue;
      } else {
        out->outcome = 1;
        out->reason = PK_R_UNREACHABLE__2_BIT_KEY;
        out->consumed_bits = off;
        return 1;
      }
    }
    case PK_S_INITIAL_MARK: {
      out->m_kind = (1ULL) & 0x7ULL;
      state = PK_S_PARSE_TOK_LEAD;
      continue;
    }
    case PK_S_ZERO_RTT_MARK: {
      out->m_kind = (2ULL) & 0x7ULL;
      state = PK_S_PARSE_LEN_LEAD;
      continue;
    }
    case PK_S_HANDSHAKE_MARK: {
      out->m_kind = (3ULL) & 0x7ULL;
      state = PK_S_PARSE_LEN_LEAD;
      continue;
    }
    case PK_S_RETRY_ACCEPT: {
      out->m_kind = (4ULL) & 0x7ULL;
      out->outcome = 0;
      out->reason = PK_R_NONE;
      out->consumed_bits = off;
      return 0;
    }
    case PK_S_PARSE_TOK_LEAD: {
      out->tok_lead_present = 1;
      if (off + 2 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->tok_lead.prefix = (uint8_t)pk_read_bits(buf, bit_len, off, 2);
      off += 2;
      if (off + 6 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->tok_lead.v6 = (uint8_t)pk_read_bits(buf, bit_len, off, 6);
      off += 6;
      uint64_t key0 = (uint64_t)out->tok_lead.prefix;
      if (key0 == 0ULL) {
        state = PK_S_TOK_W1;
        continue;
      } else if (key0 == 1ULL) {
        state = PK_S_TOK_W2;
        continue;
      } else if (key0 == 2ULL) {
        state = PK_S_TOK_W4;
        continue;
      } else if (key0 == 3ULL) {
        state = PK_S_TOK_W8;
        continue;
      } else {
        out->outcome = 1;
        out->reason = PK_R_UNREACHABLE__2_BIT_KEY;
        out->consumed_bits = off;
        return 1;
      }
    }
    case PK_S_TOK_W1: {
      out->tok0_present = 1;
      {
        uint64_t vlen = (uint64_t)out->tok_lead.v6;
        if (vlen > (bit_len - off) / 8) {
          out->outcome = 1;
          out->reason = PK_R_OUT_OF_BOUNDS;
          out->consumed_bits = off;
          return 1;
        }
        out->tok0.body_bit_off = off;
        out->tok0.body_bit_len = vlen * 8;
        off += vlen * 8;
      }
      out->m_token_len = (uint64_t)out->tok_lead.v6;
      state = PK_S_PARSE_LEN_LEAD;
      continue;
    }
    case PK_S_TOK_W2: {
      out->tok1_present = 1;
      if (off + 8 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->tok1.t = (uint8_t)((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK]);
      off += 8;
      {
        uint64_t vlen = (((uint64_t)out->tok_lead.v6 << 8ULL) | (uint64_t)out->tok1.t);
        if (vlen > (bit_len - off) / 8) {
          out->outcome = 1;
          out->reason = PK_R_OUT_OF_BOUNDS;
          out->consumed_bits = off;
          return 1;
        }
        out->tok1.body_bit_off = off;
        out->tok1.body_bit_len = vlen * 8;
        off += vlen * 8;
      }
      out->m_token_len = (((uint64_t)out->tok_lead.v6 << 8ULL) | (uint64_t)out->tok1.t);
      state = PK_S_PARSE_LEN_LEAD;
      continue;
    }
    case PK_S_TOK_W4: {
      out->tok2_present = 1;
      if (off + 24 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->tok2.t = (uint32_t)(((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK] << 16) | ((uint64_t)buf[((off >> 3) + 1) & PK_BUF_MASK] << 8) | (uint64_t)buf[((off >> 3) + 2) & PK_BUF_MASK]);
      off += 24;
      {
        uint64_t vlen = (((uint64_t)out->tok_lead.v6 << 24ULL) | (uint64_t)out->tok2.t);
        if (vlen > (bit_len - off) / 8) {
          out->outcome = 1;
          out->reason = PK_R_OUT_OF_BOUNDS;
          out->consumed_bits = off;
          return 1;
        }
        out->tok2.body_bit_off = off;
        out->tok2.body_bit_len = vlen * 8;
        off += vlen * 8;
      }
      out->m_token_len = (((uint64_t)out->tok_lead.v6 << 24ULL) | (uint64_t)out->tok2.t);
      state = PK_S_PARSE_LEN_LEAD;
      continue;
    }
    case PK_S_TOK_W8: {
      out->tok3_present = 1;
      if (off + 56 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->tok3.t = (uint64_t)(((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK] << 48) | ((uint64_t)buf[((off >> 3) + 1) & PK_BUF_MASK] << 40) | ((uint64_t)buf[((off >> 3) + 2) & PK_BUF_MASK] << 32) | ((uint64_t)buf[((off >> 3) + 3) & PK_BUF_MASK] << 24) | ((uint64_t)buf[((off >> 3) + 4) & PK_BUF_MASK] << 16) | ((uint64_t)buf[((off >> 3) + 5) & PK_BUF_MASK] << 8) | (uint64_t)buf[((off >> 3) + 6) & PK_BUF_MASK]);
      off += 56;
      {
        uint64_t vlen = (((uint64_t)out->tok_lead.v6 << 56ULL) | (uint64_t)out->tok3.t);
        if (vlen > (bit_len - off) / 8) {
          out->outcome = 1;
          out->reason = PK_R_OUT_OF_BOUNDS;
          out->consumed_bits = off;
          return 1;
        }
        out->tok3.body_bit_off = off;
        out->tok3.body_bit_len = vlen * 8;
        off += vlen * 8;
      }
      out->m_token_len = (((uint64_t)out->tok_lead.v6 << 56ULL) | (uint64_t)out->tok3.t);
      state = PK_S_PARSE_LEN_LEAD;
      continue;
    }
    case PK_S_PARSE_LEN_LEAD: {
      out->len_lead_present = 1;
      if (off + 2 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->len_lead.prefix = (uint8_t)pk_read_bits(buf, bit_len, off, 2);
      off += 2;
      if (off + 6 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->len_lead.v6 = (uint8_t)pk_read_bits(buf, bit_len, off, 6);
      off += 6;
      uint64_t key0 = (uint64_t)out->len_lead.prefix;
      if (key0 == 0ULL) {
        state = PK_S_LEN_W1;
        continue;
      } else if (key0 == 1ULL) {
        state = PK_S_LEN_W2;
        continue;
      } else if (key0 == 2ULL) {
        state = PK_S_LEN_W4;
        continue;
      } else if (key0 == 3ULL) {
        state = PK_S_LEN_W8;
        continue;
      } else {
        out->outcome = 1;
        out->reason = PK_R_UNREACHABLE__2_BIT_KEY;
        out->consumed_bits = off;
        return 1;
      }
    }
    case PK_S_LEN_W1: {
      out->m_length = (uint64_t)out->len_lead.v6;
      out->outcome = 0;
      out->reason = PK_R_NONE;
      out->consumed_bits = off;
      return 0;
    }
    case PK_S_LEN_W2: {
      out->len1_present = 1;
      if (off + 8 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->len1.t = (uint8_t)((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK]);
      off += 8;
      out->m_length = (((uint64_t)out->len_lead.v6 << 8ULL) | (uint64_t)out->len1.t);
      out->outcome = 0;
      out->reason = PK_R_NONE;
      out->consumed_bits = off;
      return 0;
    }
    case PK_S_LEN_W4: {
      out->len2_present = 1;
      if (off + 24 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->len2.t = (uint32_t)(((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK] << 16) | ((uint64_t)buf[((off >> 3) + 1) & PK_BUF_MASK] << 8) | (uint64_t)buf[((off >> 3) + 2) & PK_BUF_MASK]);
      off += 24;
      out->m_length = (((uint64_t)out->len_lead.v6 << 24ULL) | (uint64_t)out->len2.t);
      out->outcome = 0;
      out->reason = PK_R_NONE;
      out->consumed_bits = off;
      return 0;
    }
    case PK_S_LEN_W8: {
      out->len3_present = 1;
      if (off + 56 > bit_len) {
        out->outcome = 1;
        out->reason = PK_R_OUT_OF_BOUNDS;
        out->consumed_bits = off;
        return 1;
      }
      out->len3.t = (uint64_t)(((uint64_t)buf[((off >> 3) + 0) & PK_BUF_MASK] << 48) | ((uint64_t)buf[((off >> 3) + 1) & PK_BUF_MASK] << 40) | ((uint64_t)buf[((off >> 3) + 2) & PK_BUF_MASK] << 32) | ((uint64_t)buf[((off >> 3) + 3) & PK_BUF_MASK] << 24) | ((uint64_t)buf[((off >> 3) + 4) & PK_BUF_MASK] << 16) | ((uint64_t)buf[((off >> 3) + 5) & PK_BUF_MASK] << 8) | (uint64_t)buf[((off >> 3) + 6) & PK_BUF_MASK]);
      off += 56;
      out->m_length = (((uint64_t)out->len_lead.v6 << 56ULL) | (uint64_t)out->len3.t);
      out->outcome = 0;
      out->reason = PK_R_NONE;
      out->consumed_bits = off;
      return 0;
    }
    }
  }
  out->outcome = 1;
  out->reason = PK_R_MAX_DEPTH_EXCEEDED;
  out->consumed_bits = off;
  return 1;
}

uint64_t pk_entry(void *mem) {
  const uint8_t *m = (const uint8_t *)mem;
  uint64_t bit_len = 0;
  uint32_t i;
  for (i = 0; i < 8; i++) bit_len |= (uint64_t)m[i] << (8 * i);
  pk_quic_initial_result_t out = {0};
  pk_quic_initial_parse_core(m + 8, bit_len, &out);
  return ((uint64_t)out.outcome << 56) | ((uint64_t)out.reason << 48) | (out.consumed_bits & 0xFFFFFFFFFFFFULL);
}
