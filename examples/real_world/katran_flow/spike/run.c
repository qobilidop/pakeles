...
struct pk_result { uint8_t outcome; uint16_t reason; uint64_t consumed_bits; };

int main(int argc, char **argv) {
    if (argc < 3) { fprintf(stderr, "usage: %s <obj> <corpus>\n", argv[0]); return 2; }
    ...
    while (fgets(line, sizeof line, cf)) {
        ...
        if (plen < 14) continue; // XDP TEST_RUN floor
        unsigned char outbuf[4096];
        LIBBPF_OPTS(bpf_test_run_opts, topts,
            .data_in = pkt, .data_size_in = (uint32_t)plen,
            .data_out = outbuf, .data_size_out = sizeof outbuf, .repeat = 1);
        if (bpf_prog_test_run_opts(prog_fd, &topts)) { fprintf(stderr, "TEST_RUN failed\n"); return 1; }
        uint32_t z = 0; struct pk_result r; memset(&r, 0, sizeof r);
        if (bpf_map_lookup_elem(out_fd, &z, &r)) { fprintf(stderr, "map read failed\n"); return 1; }
        printf("%s  {\"outcome\": \"%s\", \"reason\": %u, \"consumed_bits\": %llu}",
               first ? "" : ",\n", r.outcome == 0 ? "accept" : "reject",
               r.reason, (unsigned long long)r.consumed_bits);
        first = 0;
    }
    fclose(cf);
    printf("\n]\n");
