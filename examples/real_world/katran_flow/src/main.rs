...
//! diffs our katran_flow parse (flow keys + XDP verdict)
...
#[command(
    version,
    about = "Diff our katran_flow parse (flow keys + XDP verdict) against the committed katran-minted golden"
)]
struct Args {
...
fn main() -> anyhow::Result<()> {
    let args = Args::parse();
    let report =
        pakeles_example_katran_flow::cli_diff(args.ir.as_deref(), args.goldens.as_deref())?;
    print!("{report}");
