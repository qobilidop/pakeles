    ...
    Ok(match e.kind.as_ref().context("empty expression")? {
        ...
        pb::expr::Kind::Bin(b) => {
            ...
            match pb::BinOpKind::try_from(b.op) {
                ...
                Ok(pb::BinOpKind::Sub) => {
                    (lmin.saturating_sub(rmax), lmax.saturating_sub(rmin))
                }
                Ok(pb::BinOpKind::Mul) => {
                ...
                Ok(pb::BinOpKind::Shl) => (
                    ...
                    lmax.checked_shl(rmax.min(64) as u32).context("shl overflow")?,
                ),
...
fn target_p4(t: &pb::Target) -> Result<String> {
    Ok(match t.kind.as_ref().context("empty target")? {
        ...
        pb::target::Kind::Reject(_) => "reject".into(),
    })
}
...
pub fn generate_p4(ir: &pb::Ir) -> Result<String> {
    ...
    if insts.len() > 8 {
        bail!("verdict bitmap supports at most 8 header instances, got {}", insts.len());
    }
    ...
    writeln!(w, "/* Generated by pakeles from `{}`. Do not edit:", parser.name)?;
        ...
        for (i, seg) in segments(ht).iter().enumerate() {
            match seg {
                ...
                Seg::Var(f) => {
                    ...
                    if max_bits > 65535 {
                        bail!("varbit bound for `{}.{}` too large: {max_bits} bits", inst, f.name);
                    }
    ...
    for s in &parser.states {
        ...
        match s.transition.as_ref().and_then(|t| t.kind.as_ref()) {
            ...
            Some(pb::transition::Kind::Select(sel)) => {
                ...
                let dt = sel.default_target.as_ref().context("select has no default")?;
    ...
    writeln!(w, "control PkVerifyChecksum(inout headers hdr, inout metadata meta) {{")?;
    writeln!(w, "    apply {{ }}")?;
    writeln!(w, "}}")?;
    writeln!(w)?;
    writeln!(
    ...
    writeln!(w, "}}")?;
    writeln!(w)?;
    writeln!(w, "control PkComputeChecksum(inout headers hdr, inout metadata meta) {{")?;
    writeln!(w, "    apply {{ }}")?;
    writeln!(w, "}}")?;
    writeln!(w)?;
