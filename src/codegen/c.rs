...
fn expr_c(e: &pb::Expr) -> Result<String> {
    match e.kind.as_ref() {
        ...
        Some(pb::expr::Kind::Field(r)) => {
            Ok(format!("(uint64_t)out->{}.{}", r.header, r.field))
        }
        Some(pb::expr::Kind::Bin(b)) => {
            let l = expr_c(b.lhs.as_deref().ok_or_else(|| anyhow::anyhow!("binop missing lhs"))?)?;
            let r = expr_c(b.rhs.as_deref().ok_or_else(|| anyhow::anyhow!("binop missing rhs"))?)?;
...
impl<'a> Emit<'a> {
    ...
    fn reason_code(&self, reason: &str) -> u32 {
        self.reasons.iter().find(|(r, _)| r == reason).map(|(_, c)| *c).unwrap_or(0)
    }
    ...
    fn header(&self) -> Result<String> {
        ...
        writeln!(w, "/* Generated by pakeles from `{}`. Do not edit:", self.parser.name)?;
        ...
        writeln!(w)?;
        writeln!(w, "enum {{ {}_ACCEPT = 0, {}_REJECT = 1 }};", p.to_uppercase(), p.to_uppercase())?;
        writeln!(w)?;
    ...
    fn core(&self, static_qual: &str) -> Result<String> {
        ...
        writeln!(w, "    v = (v << 1) | (uint64_t)((buf[pos >> 3] >> (7 - (pos & 7))) & 1);")?;
        writeln!(w, "  }}")?;
        writeln!(w, "  return v;")?;
        writeln!(w, "}}")?;
        writeln!(w)?;
        ...
        writeln!(w, "  uint32_t state = PK_S_{};", self.parser.start_state.to_uppercase())?;
        writeln!(w, "  uint32_t depth;")?;
        writeln!(w, "  for (depth = 0; depth < {}u; depth++) {{", self.parser.max_depth)?;
            ...
            Some(pb::transition::Kind::Select(sel)) => {
                ...
                for (i, arm) in sel.arms.iter().enumerate() {
                    ...
                    self.emit_target(
                        w,
                        "        ",
                        arm.next.as_ref().ok_or_else(|| anyhow::anyhow!("arm without target"))?,
                    )?;
                }
...
pub fn generate_c(ir: &pb::Ir) -> Result<CArtifacts> {
    ...
    Ok(CArtifacts { header: emit.header()?, source: emit.source()? })
}
...
pub fn generate_c_harness(ir: &pb::Ir) -> Result<String> {
    ...
    let p = &emit.prefix;
    let mut w = String::new();
    writeln!(w, "/* Generated conformance harness for `{}`. */", parser.name)?;
    writeln!(w, "#include \"parser.h\"")?;
    ...
    writeln!(w, "    hex[0] = 0;")?;
    writeln!(w, "    if (sscanf(line, \"%llu %279999s\", &bit_len, hex) < 1) continue;")?;
    writeln!(w, "    size_t nb = strlen(hex) / 2;")?;
    writeln!(w, "    if (hex[0] == '-') nb = 0;")?;
    writeln!(w, "    for (size_t i = 0; i < nb; i++)")?;
    writeln!(w, "      buf[i] = (uint8_t)((hexval(hex[2 * i]) << 4) | hexval(hex[2 * i + 1]));")?;
    ...
    for (inst, ht_name) in instances(parser) {
        let ht = parser.header_types.iter().find(|h| h.name == ht_name).unwrap();
...
pub fn generate_ebpf(ir: &pb::Ir) -> Result<String> {
    ...
    let p = &emit.prefix;
    let mut w = String::new();
    writeln!(w, "/* Generated by pakeles from `{}` (eBPF variant). Do not edit:", parser.name)?;
    writeln!(w, " * regenerate with `pakeles gen ebpf`.")?;
    writeln!(w, " * Compile: clang -O2 -target bpf -c this_file.c")?;
    writeln!(w, " * Entry contract (rbpf raw VM): r1 = mem = 8-byte LE bit_len ++ packet;")?;
    writeln!(w, " * the length prefix is the harness framing (rbpf passes no length).")?;
    writeln!(w, " * Returns outcome(8b) << 56 | reason(8b) << 48 | consumed_bits(48b).")?;
    writeln!(w, " * Note: the result struct lives on the (512-byte) BPF stack;")?;
    ...
    writeln!(w, "  uint32_t i;")?;
    writeln!(w, "  for (i = 0; i < 8; i++) bit_len |= (uint64_t)m[i] << (8 * i);")?;
...
mod tests {
    ...
    fn c_backend_conformance_full_suite() {
        ...
        let cc = std::process::Command::new("cc")
            .args(["-std=c99", "-Wall", "-Wextra", "-Werror", "-O2", "parser.c", "main.c", "-o", "harness"])
            .current_dir(&dir)
            .output()
            .unwrap();
        assert!(cc.status.success(), "cc: {}", String::from_utf8_lossy(&cc.stderr));
        ...
        for ((line, vector), bits) in lines.iter().zip(&suite.vectors).zip(&bits_list) {
            ...
            let c_fields: std::collections::HashMap<&str, &str> = parts
    ...
    fn ebpf_backend_conformance_full_suite() {
        ...
        let cc = std::process::Command::new("clang")
            .args(["-O2", "-target", "bpf", "-Werror", "-c", "ebpf.c", "-o", "ebpf.o"])
            .current_dir(&dir)
            .output()
            .unwrap();
        assert!(cc.status.success(), "clang: {}", String::from_utf8_lossy(&cc.stderr));
        let oc = std::process::Command::new("llvm-objcopy")
            .args(["-O", "binary", "--only-section=.text", "ebpf.o", "ebpf.bin"])
            .current_dir(&dir)
            .output()
            .unwrap();
        assert!(oc.status.success(), "objcopy: {}", String::from_utf8_lossy(&oc.stderr));
        ...
        for v in &suite.vectors {
            ...
            match &reference.outcome {
                ...
                crate::interp::Outcome::Reject { reason } if outcome == 1 && reason == reason_str => {}
