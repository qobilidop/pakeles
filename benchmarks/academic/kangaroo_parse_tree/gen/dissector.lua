-- Generated by pakeles from `kangaroo_parse_tree` (ir 0.2.0).
...
local p = Proto("pakeles_kangaroo_parse_tree", "Pakeles kangaroo_parse_tree")

local ef_error = ProtoExpert.new("pakeles_kangaroo_parse_tree.malformed", "Malformed", expert.group.MALFORMED, expert.severity.ERROR)
...
function states.parse_recirc(buf, pinfo, tree, off, depth)
  depth = depth + 1
  if depth > 13 then
    tree:add_proto_expert_info(ef_error, "max depth exceeded")
    return off
...
function states.parse_service(buf, pinfo, tree, off, depth)
  depth = depth + 1
  if depth > 13 then
    tree:add_proto_expert_info(ef_error, "max depth exceeded")
    return off
...
function states.parse_ipv6_ext(buf, pinfo, tree, off, depth)
  depth = depth + 1
  if depth > 13 then
    tree:add_proto_expert_info(ef_error, "max depth exceeded")
    return off
  ...
  off = off + 8
  local len_ipv6_ext_hdr_body = ((((1 + v.v_ipv6_ext_hdr_hdr_ext_len) * 2 ^ 3) - 2) * 8)
  if len_ipv6_ext_hdr_body < 0 or off + len_ipv6_ext_hdr_body > avail then
    hdr_ipv6_ext_hdr:add_proto_expert_info(ef_error, "out of bounds in ipv6_ext_hdr.body")
    return off
  end
  if len_ipv6_ext_hdr_body > 0 then
    hdr_ipv6_ext_hdr:add(pf.f_ipv6_ext_hdr_body, buf(math.floor(off / 8), math.floor(len_ipv6_ext_hdr_body / 8)))
  end
  off = off + len_ipv6_ext_hdr_body
...
function states.parse_gre(buf, pinfo, tree, off, depth)
  ...
  off = off + 16
  local len_gre_opt = ((((v.v_gre_c * 4) + (v.v_gre_k * 4)) + (v.v_gre_s * 4)) * 8)
  if len_gre_opt < 0 or off + len_gre_opt > avail then
    hdr_gre:add_proto_expert_info(ef_error, "out of bounds in gre.opt")
    return off
  end
  if len_gre_opt > 0 then
    hdr_gre:add(pf.f_gre_opt, buf(math.floor(off / 8), math.floor(len_gre_opt / 8)))
  end
  off = off + len_gre_opt
...
function states.parse_ipv4_inner(buf, pinfo, tree, off, depth)
  depth = depth + 1
  if depth > 13 then
    tree:add_proto_expert_info(ef_error, "max depth exceeded")
    return off
...
function states.parse_ipv6_inner(buf, pinfo, tree, off, depth)
  depth = depth + 1
  if depth > 13 then
    tree:add_proto_expert_info(ef_error, "max depth exceeded")
    return off
