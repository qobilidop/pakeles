...
static int pk_gibb_edge_parse_core(const uint8_t *buf, uint64_t bit_len, pk_gibb_edge_result_t *out) {
  uint64_t off = 0;
...
int pk_gibb_edge_parse(const uint8_t *buf, uint64_t bit_len, pk_gibb_edge_result_t *out) {
  pk_gibb_edge_result_t zero = {0};
  *out = zero;
  return pk_gibb_edge_parse_core(buf, bit_len, out);
}
