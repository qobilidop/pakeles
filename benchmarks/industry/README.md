# `industry/` — descriptions checked against real implementations

Each member here models a parser that already exists in the world, and
is verified to agree with it **packet for packet, at a pinned version**.
These double as the industry half of the benchmark corpus (see
`../academic/` for the other half) — but "benchmark" is the corpus
role, not the identity: each of these is an incumbent-agreement CLAIM,
with the oracle as the boss.
...
`pakeles-benchmark-<name>`), so **everything about one incumbent lives in
...
`cargo run -p pakeles-benchmark-<name>`, or just `cargo run` from inside
the directory). `cargo test -p pakeles-benchmark-<name>` runs one
