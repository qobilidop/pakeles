...
int main(int argc, char **argv) {
  ...
  while (fgets(line, sizeof line, cf)) {
    ...
    if (plen <= 0) {
      fprintf(stderr, "bad corpus line: %s", line);
      return 1;
    }
    ...
    printf(
        "%s    {\"packet_hex\": \"%s\", \"ptype\": %u, \"ptype_name\": \"%s\", "
        "\"hdr_lens\": {\"l2_len\": %u, \"l3_len\": %u, \"l4_len\": %u, "
        "\"tunnel_len\": %u, \"inner_l2_len\": %u, \"inner_l3_len\": %u, "
        "\"inner_l4_len\": %u}}",
        first ? "" : ",\n", phex, ptype, name, hl.l2_len, hl.l3_len, hl.l4_len,
        hl.tunnel_len, hl.inner_l2_len, hl.inner_l3_len, hl.inner_l4_len);
    first = 0;
  }
  fclose(cf);
  printf("\n  ]\n}\n");
