/* Generated by pakeles from `katran_parser` (eBPF variant). Do not edit:
...
static __attribute__((always_inline)) int pk_katran_parser_parse_core(const uint8_t *buf, uint64_t bit_len, pk_katran_parser_result_t *out) {
  uint64_t off = 0;
...
uint64_t pk_entry(void *mem) {
  ...
  pk_katran_parser_result_t out = {0};
  pk_katran_parser_parse_core(m + 8, bit_len, &out);
