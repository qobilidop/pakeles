...
hand with `cargo run -p pakeles-benchmark-katran-parser`) diffs our
projection ([`src/lib.rs`](src/lib.rs))
against the committed golden. Re-minting is privileged (in-kernel
TEST_RUN):

```sh
./dev-priv.sh benchmarks/industry/katran_parser/factory/capture.sh
```
...
```sh
./dev.sh cargo run -p pakeles-benchmark-katran-parser   # our keys+verdict vs the committed golden
```
