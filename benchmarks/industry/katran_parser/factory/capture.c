...
int main(int argc, char **argv) {
  if (argc < 3) {
    fprintf(stderr, "usage: %s <balancer.o> <corpus.txt>\n", argv[0]);
    return 2;
  }
  ...
  struct bpf_program *entry =
      bpf_object__find_program_by_name(obj, "balancer_ingress");
  if (!entry) {
    fprintf(stderr, "missing program balancer_ingress — pin drift?\n");
    return 1;
  }
  ...
  if (!xmap) {
    fprintf(stderr, "missing pk_export_map — observation patch not applied?\n");
    return 1;
  }
  ...
  while (fgets(line, sizeof line, cf)) {
    ...
    printf("%s    {\"packet_hex\": \"%s\", \"verdict\": \"%s\", \"out_hex\": "
           "\"%s\", "
           "\"stage\": %u",
    ...
    if (ex.stage & 1) {
      ...
      printf(", \"flow\": {\"src\": \"%s\", \"dst\": \"%s\", "
             ...
             src, dst, ntohs(ex.pckt.flow.port16[0]),
             ntohs(ex.pckt.flow.port16[1]), ex.pckt.flow.proto, ex.pckt.flags,
             ex.pckt.tos, (ex.stage & 2) ? "true" : "false");
    }
    if (ex.stage & 4) {
