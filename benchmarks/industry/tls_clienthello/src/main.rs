...
//! diffs our tls_clienthello parse (verdict class + SNI)
//! against the committed rustls-minted golden — the human-facing form of
...
#[command(
    version,
    about = "Diff our tls_clienthello parse (verdict class + SNI) against the committed rustls-minted golden"
)]
struct Args {
...
fn main() -> anyhow::Result<()> {
    let args = Args::parse();
    let report =
        pakeles_benchmark_tls_clienthello::cli_diff(args.ir.as_deref(), args.goldens.as_deref())?;
    print!("{report}");
