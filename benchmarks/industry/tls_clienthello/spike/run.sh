...
#
#   ./dev-priv.sh benchmarks/industry/tls_clienthello/spike/run.sh          # committed IR (max_depth 96)
#   PK_DEPTH=22 ./dev-priv.sh benchmarks/industry/tls_clienthello/spike/run.sh
#
...
EOF
  (cd ../../../.. && cargo run --quiet --bin pakeles -- gen bpf \
     --ir benchmarks/industry/tls_clienthello/spike/build/depth/ir.json \
     --out benchmarks/industry/tls_clienthello/spike/build/depth/parser.bpf.c)
  (cd ../../../.. && cargo run --quiet --bin pakeles -- gen c \
     --ir benchmarks/industry/tls_clienthello/spike/build/depth/ir.json \
     --out-dir benchmarks/industry/tls_clienthello/spike/build/depth)
  gen=build/depth
