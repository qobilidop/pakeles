...
int main(int argc, char **argv) {
  ...
  while (fgets(line, sizeof line, cf)) {
    ...
    if (plen < 14)
      continue; /* XDP TEST_RUN floor, mirrored */
    if (plen > 512)
      plen = 512; /* scratch cap, mirrored */
    pk_tls_clienthello_result_t r;
    memset(&r, 0, sizeof r);
    pk_tls_clienthello_parse(pkt, (unsigned long long)plen * 8, &r);
