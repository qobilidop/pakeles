...
#
#   ./dev-priv.sh benchmarks/industry/tls_clienthello/spike/depth-sweep.sh
#
...
EOF
  (cd ../../../.. && cargo run --quiet --bin pakeles -- gen bpf \
     --ir "benchmarks/industry/tls_clienthello/spike/build/sweep/ir-$d.json" \
     --out "benchmarks/industry/tls_clienthello/spike/build/sweep/parser-$d.bpf.c")
