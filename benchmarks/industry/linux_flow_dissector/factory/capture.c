...
static void hexcat(char *dst, const unsigned char *b, int n) {
  for (int i = 0; i < n; i++)
    sprintf(dst + i * 2, "%02x", b[i]);
  dst[n * 2] = 0;
}

int main(int argc, char **argv) {
  if (argc < 3) {
    fprintf(stderr, "usage: %s <bpf_flow.o> <corpus.txt>\n", argv[0]);
    return 2;
  }
  ...
  if (bpf_object__load(obj)) {
    fprintf(stderr, "load failed (need privilege?)\n");
    return 1;
  }

  struct bpf_map *jmp = bpf_object__find_map_by_name(obj, "jmp_table");
  if (!jmp) {
    fprintf(stderr, "no jmp_table map — pin drift?\n");
    return 1;
  }
  for (uint32_t i = 0; i < MAX_PROG; i++) {
    ...
    if (!p) {
      fprintf(stderr, "missing program %s — pin drift?\n", name);
      return 1;
    }
  ...
  if (!entry) {
    fprintf(stderr, "missing entry program _dissect — pin drift?\n");
    return 1;
  }
  ...
  printf(
      "  \"keys_subset\": "
      "[\"nhoff\",\"thoff\",\"n_proto\",\"addr_proto\",\"ip_proto\","
  ...
  while (fgets(line, sizeof line, cf)) {
    ...
    LIBBPF_OPTS(bpf_test_run_opts, topts, .data_in = pkt,
                .data_size_in = (uint32_t)plen, .data_out = out,
    ...
    printf("%s    {\"packet_hex\": \"%s\", \"disposition\": \"ok\", \"keys\": {"
           ...
           "\"is_encap\": %s}}",
           first ? "" : ",\n", phex, k->nhoff, k->thoff, ntohs(k->n_proto),
           k->addr_proto, k->ip_proto, ntohs(k->sport), ntohs(k->dport), v4s,
           v4d, v6s, v6d, ntohl(k->flow_label), k->is_frag ? "true" : "false",
           k->is_first_frag ? "true" : "false", k->is_encap ? "true" : "false");
    first = 0;
  }
  fclose(cf);
  printf("\n  ]\n}\n");
